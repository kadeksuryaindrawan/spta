        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                    <div class="col-10">
                        <h4 class="card-title">Edit Bimbingan Proposal</h4>
                    </div>
                    </div>
                    ...
                    <?php if($this->session->flashdata('error')): ?>
							...
							<?php endif ?>
                        <form action="<?= base_url('ProposalController/editBimbinganProcess/').$bimbingan['bimbingan_proposal_id'] ?>" method="POST"  enctype="multipart/form-data">
                            <input type="hidden" value="<?= $bimbingan['bimbingan_proposal_id'] ?>" name="bimbingan_proposal_id">
                            <div class="form-group">
                                ...
                                <select name="proposal_id" id="proposal" class="form-control input-default">
                                    ...
                                    foreach ($proposal->result_array() as $key): 
                                        $selected = ($key['proposal_id'] == $bimbingan['proposal_id']) ? 'selected' : '';
                                    ?>
                                        <option value="<?= $key['proposal_id'] ?>" <?= $selected ?>><?= $key['judul'] ?></option>
		                            <?php endforeach ?>
                                </select>
                            </div>
